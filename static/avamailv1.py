...
def format_sentiment_output_html(data: dict, chat_transcript: str, customer_name: str, conversation_date: str) -> str:
    def render_expectations(expectations):
        ...
        for item in expectations:
            ...
            <tr>
                ...
                <td>{item.get('notes')} {"(Gap)" if item.get("gap") else ""}</td>
            </tr>
            """
        return rows or "<tr><td colspan='4'>No expectations shared.</td></tr>"

    def render_team_feedback(feedback):
        rows = ""
        for member in feedback:
            rows += f"""
            <tr>
                <td>{member.get('name')}</td>
                <td>{member.get('role')}</td>
                <td>{member.get('rating')}</td>
                <td>{member.get('strengths')}</td>
                <td>{member.get('improvement')}</td>
            </tr>
            """
        return rows or "<tr><td colspan='5'>No team feedback.</td></tr>"
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6;">
        ...
        <p>I have just completed a feedback conversation with our customer, <strong>{customer_name.split()[0]}</strong>, and I would like to share my <strong>analysis and recommendations</strong> based on that discussion.<p>
        ...
        <p><strong>Sentiment:</strong> {data.get("overall_sentiment", "Unknown")}<br>
        <strong>Key Insight:</strong> {data.get("business_insight", "N/A")}<br>
        <strong>Business Posture:</strong> {data.get("business_posture", "N/A")}</p>

        <h3>2. Expectations from Clarion</h3>
        <table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse; width: 100%;">
            <tr>
                <th>Priority</th><th>Expectation</th><th>Rating (1–10)</th><th>Notes / Gap Identified</th>
            </tr>
        ...
        <table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse; width: 100%;">
            <tr>
                <th>Name</th><th>Role</th><th>Rating</th><th>Strengths</th><th>Areas for Improvement</th>
            </tr>
            {render_team_feedback(data.get("team_feedback", []))}
        </table>

        <p><strong>Manager ({data.get("manager_name", "N/A")}):</strong> {data.get("manager_summary", "N/A")}<br>
        <strong>SDM ({data.get("sdm_name", "N/A")}):</strong> {data.get("sdm_summary", "N/A")}</p>
        ...
        <p>Regards,<br><em>Ava<br>Virtual Assistant – Clarion Technologies</em></p>
    </body>
    </html>
    """
